...
- Persona topic cards use a two-axis visual system. `data-skin` stores the restrained color family for a broad topic (`reading`, `learning`, `systems`, or `history`); `data-pattern` stores the series-specific linework, and `data-series` stores a stable kebab-case series key. Articles in the same series reuse all three values, while unrelated books or series may share a color family but must not silently share the same pattern. Add a concise visible `.persona-entry-series` label so the relationship is legible without decoding the decoration. Treat `Reading Notes` as a content family rather than a series name; for book-derived writing, use the book or an explicitly named multi-article project as the series. Mark series order in the visible title with `(n/total)` or `（n/total）` only when a multi-article total is actually planned; avoid internal codes like `A1` and speculative `(1/?)` labels. Keep the effect subtle: accent edge, light pattern, or linework rather than full-card color blocks.
...
- Every published Project must define one primary `highlight` and one or two `supportingHighlights`. The shared Project layout gives the primary contribution the strongest visual weight. Choose a genuinely memorable analytical, technical, content, or product contribution, then use supporting highlights for evidence, method, delivery, or a key interpretation boundary. Write body headings as reader-facing questions, decisions, or takeaways rather than generic research-report labels.
